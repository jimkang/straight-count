...
const graphvizHeader = `digraph {
  graph [fontsize=12, fontname=Ubuntu, penwidth=3];
  node [fontsize=12, fontname=Ubuntu, shape=msquare, style=filled, color="#ffffff"];
...
for (let gen = 0; gen < 2; ++gen) {
  ...
  for (let hand of prevGen) {
    ...
    for (let nextHand of nextHands) {
      process.stdout.write(`"${handToString(hand)}" -> "${handToString(nextHand)}";\n`);
